...
describe("Change Password Component UI Tests", () => {
  ...
  it("AUTH-UI-03: enters busy state during password update submission", async () => {
    let resolveChange: (val: any) => void;
    const changePromise = new Promise((resolve) => {
      resolveChange = resolve;
    });
    vi.spyOn(api, "changePassword").mockReturnValue(changePromise as any);
  ...
  it("AUTH-UI-03: displays server error banner when API returns error", async () => {
    vi.spyOn(api, "changePassword").mockRejectedValue(
      new Error("Password must be at least 8 characters long, contain uppercase, lowercase, number, and special character")
    );
  ...
  it("AUTH-UI-03: calls onSuccess when password is changed successfully", async () => {
    ...
    render(<ChangePassword onSuccess={handleSuccess} onLogout={vi.fn()} />);
  ...
  it("AUTH-UI-03: provides a logout action to abort mandatory password change", async () => {
    ...
    render(<ChangePassword onSuccess={vi.fn()} onLogout={handleLogout} />);
