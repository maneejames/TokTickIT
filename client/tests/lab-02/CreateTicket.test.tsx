...
describe("Create Ticket Component UI Tests", () => {
  ...
  const renderComponent = () => {
    return render(
      <CreateTicket currentRequester={mockRequester} />
    );
  };
  ...
  describe("TICK-UI-02: Submission failure preserves form inputs", () => {
    it("shows error callout on 500 API failure and preserves entered form values", async () => {
      ...
      expect((screen.getByLabelText(/Related System/i) as HTMLSelectElement).value).toBe("7");
      expect((screen.getByLabelText(/Ticket Summary/i) as HTMLInputElement).value).toBe("Laptop screen flickering intermittently");
      expect((screen.getByLabelText(/Problem Description/i) as HTMLTextAreaElement).value).toBe("The screen flickers whenever brightness is adjusted above 50%.");
    });
  });
  ...
  describe("TICK-UI-04: Submission success confirmation", () => {
    it("displays generated ticket number and stub link to ticket details on success", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByRole("link", { name: /View Ticket Details/i })).toHaveAttribute(
          "href",
          "/tickets/101"
        );
      });
