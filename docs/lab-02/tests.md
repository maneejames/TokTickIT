...
| **TICK-UNIT-01** | Unit | BR-01, AC-05 | Ticket number generator format | Generates `TICK-YYYYMMDD-XXXX` with zero-padded counter | `server/tests/lab-02/create-ticket.api.test.ts` | Planned |
| **TICK-API-01** | API | AC-05 | Create valid ticket | 201 Created; status=NEW; unique ticket number; saved in DB | `server/tests/lab-02/create-ticket.api.test.ts` | Planned |
| **TICK-API-02** | API | AC-06, AC-07 | Summary/Description validation | 400 Bad Request if summary < 5 chars or description > 2000 chars | `server/tests/lab-02/create-ticket.api.test.ts` | Planned |
| **TICK-API-03** | API | AC-06 | Whitespace trimming before validation | Spaces only (e.g. `" "`) rejected with 400 Bad Request | `server/tests/lab-02/create-ticket.api.test.ts` | Planned |
| **TICK-UI-01** | UI | AC-07 | Form validation errors | Red messages appear below invalid fields; API not called | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
| **TICK-UI-02** | UI | AC-09 | Submission failure preserves inputs | Error alert shown on 500 error; form values remain in fields | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
| **TICK-UI-03** | UI | AC-10 | Submit button enters busy state | Button disabled with spinner during request processing | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
| **TICK-UI-04** | UI | AC-05 | Submission success confirmation | Displays generated ticket number and link to detail | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
| **LIST-API-01** | API | AC-11 | List tickets for active requester | 200 OK; returns items matching `X-Requester-Id` | `server/tests/lab-02/my-tickets.api.test.ts` | Planned |
...
| **LIST-API-04** | API | AC-14, AC-16 | Search and sort query options | Filters by summary substring; sorts ascending/descending | `server/tests/lab-02/my-tickets.api.test.ts` | Planned |
| **LIST-UI-01** | UI | AC-11, AC-25 | Desktop table vs mobile card view | Renders table headers on desktop and stacked cards on mobile | `client/tests/lab-02/MyTickets.test.tsx` | Planned |
...
| **LIST-UI-03** | UI | AC-15 | No results search state | Displays "No matching tickets found" with "Clear Filters" button | `client/tests/lab-02/MyTickets.test.tsx` | Planned |
| **DET-API-01** | API | AC-17 | Get single ticket detail | 200 OK; returns full ticket details and attachments array | `server/tests/lab-02/ticket-detail.api.test.ts` | Planned |
| **DET-API-02** | API | AC-18 | Unauthorized ticket detail access | 403 or 404 if ticket belongs to another requester | `server/tests/lab-02/ticket-detail.api.test.ts` | Planned |
| **DET-UI-01** | UI | AC-17 | Read-only ticket detail display | Fields displayed in non-editable formatted containers | `client/tests/lab-02/RequesterTicketDetail.test.tsx` | Planned |
| **ATT-API-01** | API | AC-19 | Upload valid attachment | 201 Created; saved on disk; metadata stored in DB | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| **ATT-API-02** | API | AC-08 | Reject file > 5 MB | 413 Payload Too Large; file not saved | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| **ATT-API-03** | API | AC-08 | Reject invalid MIME type (e.g. .exe) | 415 Unsupported Media Type; rejected | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| **ATT-API-04** | API | AC-20 | Reject 6th active attachment | 400 Bad Request when 5 active attachments exist | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| **ATT-API-05** | API | AC-21 | Download active attachment | 200 OK; binary stream matching original filename header | `server/tests/lab-02/attachments.api.test.ts` | Planned |
...
| **ATT-API-07** | API | AC-24 | Block download of soft-removed file | 410 Gone / 404 Not Found when requesting removed file | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| **ATT-UI-01** | UI | AC-22, AC-23 | Soft-removal modal and reason | Confirmation modal opens; submitting updates UI to removed state | `client/tests/lab-02/AttachmentSection.test.tsx` | Planned |
...
| **AC-04** | Stored invalid requester redirected to selector | `REQ-API-02`, `REQ-UI-01` |
...
| **AC-08** | Immediate rejection of files > 5 MB or invalid MIME | `ATT-API-02`, `ATT-API-03` |
...
| **AC-14** | Search by summary and ticket number | `LIST-API-04` |
| **AC-15** | No matching results state with Clear Filters | `LIST-UI-03` |
| **AC-16** | Sorting tickets by creation date ascending/descending | `LIST-API-04` |
| **AC-17** | Read-only Ticket Detail display for owner | `DET-API-01`, `DET-UI-01`, `E2E-01` |
| **AC-18** | Rejection of unauthorized ticket detail access (403/404) | `DET-API-02` |
...
| **AC-24** | Soft-removed attachment download blocked (410/404) | `ATT-API-07` |
...
| **My Tickets List** | Full 7-column table with hover effects | Condensed table with scroll | Responsive stacked cards with badges | [ ] |
...
*(This section is updated upon completion of implementation issues #3 through #7)*

| Test Category | Total Tests | Passed | Failed | Skipped | Pass Rate |
|---|---|---|---|---|---|
| Backend API & Unit Tests | 14 | - | - | 0 | - |
| Frontend Component Tests | 11 | - | - | 0 | - |
| E2E Acceptance Tests | 1 | - | - | 0 | - |
| **Total** | **26** | - | - | **0** | **100%** |
