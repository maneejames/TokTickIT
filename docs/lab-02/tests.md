...
| **LIST-API-01** | API | AC-11 | List tickets for active requester | 200 OK; returns items matching `X-Requester-Id` with active attachments count | `server/tests/lab-02/my-tickets.api.test.ts` | Planned |
...
| **DET-API-02** | API | AC-18 | Unauthorized ticket detail access | 404 Not Found if ticket belongs to another requester | `server/tests/lab-02/ticket-detail.api.test.ts` | Planned |
...
|---|---|---|---|---|---|
| Backend API & Unit Tests | 23 | - | - | 0 | - |
| Frontend Component Tests | 15 | - | - | 0 | - |
| E2E Acceptance Tests | 1 | - | - | 0 | - |
| **Total** | **39** | - | - | **0** | **100%** |

*(Baseline draft erroneously reported 26 total tests [14 backend + 11 frontend + 1 E2E] while actually defining 33 [19 backend + 13 frontend + 1 E2E]. With the addition of 6 coverage tests [REF-API-01, REF-API-02, LIST-API-05, LIST-API-06, ATT-UI-03, REQ-UI-03], the final total is 39 tests).*
